...
# @Last version : 2022-01-08 20:40
# @Filename : res_drawPurity.py
...
item = 'TYPE3'
db = 'TCGA'
survfile = '~/Documents/MANU/BI/PANC_ECM/Manuscript/Data/Clinica_' + \
...
def MergeClassFile(datafile, classfile, inplacedict=[], has_inplace=False, has_changeid=False):
    ...
    classframe = pd.read_csv(classfile)
    classframe.columns = ['case_id', 'classname']
    if has_changeid == True:
        if '.' in dataframe['case_id'][2]:
            for i in dataframe.index:
                dataframe['case_id'][i] = _ExchangePatientID(
                    dataframe['case_id'][i])
    # if has_changeid == True:
...
if __name__ == '__main__':
    ...
    #items = ['TumorPurity']
    classdata = pd.read_csv(survfile)
    classdata = classdata[['case_id', 'lasso_result']]
    classdata.to_csv('temp.csv', index=False)
    nf = MergeClassFile(
