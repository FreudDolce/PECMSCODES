...
genefile = '~/Documents/MANU/BI/PANC_ECM/Manuscript/Data/Meta_genes.csv'
# genefile = '~/Documents/MANU/BI/PANC_ECM/Manuscript/Data/ROS_genes.csv'
geneinfo = pd.read_csv(genefile, index_col=[0])

pathway_columns = list(geneinfo.columns)
for pathway in pathway_columns:
    genelist = list(set(geneinfo[pathway]))
    if 0 in genelist:
        genelist.remove(0)
    ...
    plt.savefig('/Users/freud/Documents/MANU/BI/PANC_ECM/Manuscript/Figure/SubFigure/' +
                '_'.join(pathway.split(' ')) + '_Meta_heatmap.tiff')
